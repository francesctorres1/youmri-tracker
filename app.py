...
Tablero de flujo DEV -> PROD para las historias de usuario del backlog.
Los estados, asignaciones y fechas se guardan en Google Sheets.
"""
...
# --- Personas asignables (editable aquí) ---
PERSONAS = ["— Sin asignar —", "Francesc Torres (CSO)",
            "Ferran Illa", "Javier Nájera", "Pablo Zaragüeta"]

# --- Flujo de estados. El orden define el avance del pipeline. ---
ESTADOS = [
    "Backlog",
    "En desarrollo",
    "Revisión DEV",
...
<style>
    ...
    /* Texto general legible sobre fondo oscuro */
    ...
    /* Captions y textos secundarios: subir brillo */
    [data-testid="stCaptionContainer"], .stCaption, small {{ color:#AEB1CC !important; }}
    /* Encabezados de filtros y expanders */
    [data-testid="stExpander"] summary, .streamlit-expanderHeader {{ color:{CREMA} !important; }}
    /* Pestañas */
    ...
    /* Barra lateral: fondo claro -> texto oscuro para que se lea */
...
st.markdown(f"<h1><span class='marca'>MRIPlanexpert | YouMRI</span> · Gestión de Desarrollo</h1>",
            unsafe_allow_html=True)

# ----------------------------------------------------------------------------
# CARGA DE ESTADO (Google Sheets)
# ----------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------
# SIDEBAR — Documentos (subir / reemplazar / eliminar)
# ----------------------------------------------------------------------------
with st.sidebar:
    ...
    if docs:
        for d in docs:
            ...
            c1.markdown(f"**{d['tipo']}** · {d['nombre']}  \n"
                        ...
                        unsafe_allow_html=True)
            if c2.button("🗑️", key=f"del_{d['tipo']}_{d['nombre']}"):
...
if not state:
    st.info("👈 Empieza subiendo tu **Backlog** en la barra lateral.")
    st.stop()
...
tab1, tab2 = st.tabs(["\U0001F5C2\uFE0F Tablero de tareas", "\U0001F4CA Vista global / export"])

# ============================ TAB 1: TABLERO ================================
with tab1:
    grupos_exist = sorted({str(t.get("Grupo","")).strip()
                           for t in state if str(t.get("Grupo","")).strip()})

    with st.container(border=True):
        st.markdown("**\U0001F517 Seleccion multiple y grupos**")
        gsel1, gsel2, gsel3 = st.columns([2,2,1])
        grupo_pick = gsel1.selectbox("Seleccionar un grupo relacionado",
            ["\u2014 ninguno \u2014"] + grupos_exist, key="grupo_pick")
        ids_del_grupo = [t["ID"] for t in state
                         if str(t.get("Grupo","")).strip() == grupo_pick] \
        ...
        if gsel2.button("\u2705 Marcar historias de este grupo", use_container_width=True,
            ...
            st.rerun()
        if gsel3.button("\u2716\uFE0F Limpiar seleccion", use_container_width=True):
    ...
    if seleccionadas:
        ...
        with st.container(border=True):
            st.markdown(f"**\u26A1 Accion masiva \u2014 {len(seleccionadas)} seleccionadas:** "
                        f"{', '.join(ids_sel)}")
            b1, b2 = st.columns(2)
            ...
            g1b, g2b = st.columns([2,1])
            masa_grupo = g1b.text_input("Guardar estas historias como grupo (nombre)",
                key="masa_grupo",
                placeholder="Ej.: Semaforo, Motor evaluacion, Pipeline DICOM...")
            ...
            if st.button("\U0001F4BE Aplicar a las seleccionadas", type="primary"):
                ...
                for t in seleccionadas:
                    ...
                    if masa_grupo.strip():
                        rec["Grupo"] = masa_grupo.strip()
                    save_task(rec)
                refresh()
                st.success(f"Aplicado a {len(seleccionadas)} historias."
                           + (f" Agrupadas como \u00ab{masa_grupo.strip()}\u00bb." if masa_grupo.strip() else ""))
                st.rerun()
    ...
    for _, t in view.iterrows():
        ...
        with col_exp.expander(
            ...
            if t.get("Criterios"):
                with st.popover("Ver criterios de aceptacion"):
            ...
            if t.get("Historico"):
                with st.popover("\U0001F552 Historico de fechas"):
                    st.text(t["Historico"])

# ====================== TAB 2: VISTA GLOBAL / EXPORT ========================
with tab2:
    cols = ["ID","Épica","Fase","Estado","Asignado","Prioridad",
            "Prio_Francesc","SP","Correctivas","Historico"]
    cols = [c for c in cols if c in df.columns]
    st.dataframe(df[cols], use_container_width=True, height=520)

    buf = io.BytesIO()
    with pd.ExcelWriter(buf, engine="openpyxl") as w:
        df.drop(columns=["_ord"], errors="ignore").to_excel(
